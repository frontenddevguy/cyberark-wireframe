    ...
    $.fn.fullpage = function(options) {
        ...
        */
        ...
        $(SECTION_SEL).each(function(index){
            ...
            var parray = processMultiArray( options.anchors);
            if (typeof parray[index] !== 'undefined') {
                $(this).attr('data-anchor', parray[index]);

                //activating the menu / nav element on load
                if($(this).hasClass(ACTIVE)){
                    activateMenuAndNav(parray[index], index);
                }
            }
...
//            }
        ...
        function processMultiArray(arrToConvert){
            var newArr = [];
...
//            for (var i = 0; i < $(SECTION_SEL).length; i++) {
            for (var i = 0; i < options.anchors.length; i++) {
                ...
                if ($.isArray(options.anchors[i])) {

                        var link = '';
                        var thisOpt = options.anchors[i];
                       if (thisOpt.length) {
                                link = thisOpt[0];
                            }
